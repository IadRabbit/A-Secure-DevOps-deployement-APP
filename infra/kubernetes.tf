...
data "aws_iam_policy_document" "asdda-deployment-sa-role-assume-role-policy" {
  statement {
    effect = "Allow"
...
resource "aws_iam_role" "asdda-deployment-sa-role" {
  name               = "asdda-deployment-sa-role"
  assume_role_policy = data.aws_iam_policy_document.asdda-deployment-sa-role-assume-role-policy.json
}


data "aws_iam_policy_document" "asdda-deployment-sa-role-policy" {
  statement {
    sid = "1"
    actions = [
      "s3:ListBucket",
...
resource "aws_iam_role_policy" "asdda-deployment-sa-role-policy" {
  name   = "ssda-deployment-sa-role-policy"
  role   = aws_iam_role.asdda-deployment-sa-role.id
  policy = data.aws_iam_policy_document.asdda-deployment-sa-role-policy.json
}


resource "aws_eks_pod_identity_association" "asdda-deployment-sa-2-role" {
  ...
  role_arn        = aws_iam_role.asdda-deployment-sa-role.arn
}


resource "kubernetes_secret" "asdda-deployment-db-access" {
  metadata {
    name = "asdda-deployment-db-access"
  }
